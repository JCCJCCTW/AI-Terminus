...
struct HostListView: View {
    ...
    var groupedHosts: [(key: String, hosts: [SSHHost])] {
        let allHosts = filteredHosts.sorted { $0.displayTitle < $1.displayTitle }
        let grouped = Dictionary(grouping: filteredHosts) { host in
        ...
        return [(key: "全部", hosts: allHosts)] + explicitGroups
    }
            ...
            if isExpanded {
                ForEach(hosts) { host in
                    Button {
                        ...
                        HostRowView(
                            host: host,
                            isSelected: selectedRowId == "\(name)::\(host.id.uuidString)"
                        )
                        .padding(.leading, 20)
                        .contentShape(Rectangle())
                    }
                        .buttonStyle(.plain)
                        .contextMenu {
                            Button(appState.t("連線", "Connect")) { onConnect(host) }
                            Divider()
